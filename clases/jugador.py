...
class Jugador:
    ...
    def ganar_experiencia(self,experiencia):
        ...
        if self.experiencia >=100:
            self.nivel +=1
            self.experiencia = 0
            self.salud=100+ 1* self.nivel
            print(f"{self.nombre} ha subido de nivel a {self.nivel}")

    def descansar(self):
        if self.salud< self.salud*self.nivel:
            self.salud+=1*self.nivel
            print(f"El descanso te hizo recuperar {self.nivel*1} puntos de salud")
        else:
            print("Tenes la salud al máximo.. Sigue luchando!")
